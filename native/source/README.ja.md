...
このコネクタの既存のデスクトップアダプタソースはまだ存在しません。

このディレクトリは `irodori.dynamodb` のマイグレーションステージングエリアです。アクティブなネイティブエントリポイントは `src/lib.rs` にあり、共有ABIヘルパーは `src/abi.rs` に、エンジンの動作は `src/stub.rs` または `src/driver.rs` にあります。エンジン固有の接続/クエリ/メタデータコードは、これらのスナップショットからその動作モジュールに移動すべきです。コネクタのランタイム契約がデスクトップアプリに組み込まれるときに移行します。
...
エンジンのステータスは `knowledge/engines.json` から：`recognized_no_connector`。