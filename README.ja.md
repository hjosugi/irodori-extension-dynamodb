...
このコネクタは、クライアントが適切な資格情報フィールドを表示できるように、以下の認証モードを宣伝しています。必要に応じて、`options`を通じてドライバ固有またはプロバイダー固有の値を渡すことも可能です。

| 認証方法 | ラベル | 種類 | シークレットの用途 |
| --- | --- | --- | --- |
| `none` | 認証なし | `none` | なし |
| `connectionString` | コネクション文字列 / DSN | `connectionString` | なし |
